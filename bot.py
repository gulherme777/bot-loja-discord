...
@bot.tree.command(name="2fa", description="Gerar código 2FA")
async def twofa(interaction: discord.Interaction):
    modal = Modal2FA()
    await interaction.response.send_modal(modal)

@bot.tree.command(name="estoque", description="Verificar estoque de um produto")
async def ver_estoque(interaction: discord.Interaction, produto_id: str):
    ...
    try:
        ...
        if produto_id in produtos_disponiveis:
            ...
            await interaction.followup.send(f"📦 **{nome}** - Estoque: **{qtd}** unidades", ephemeral=True)
        else:
                ...
                if len(partes) >= 3:
                    ...
                    if user and p_id in produtos_disponiveis:
                        ...
                        asyncio.run_coroutine_threadsafe(
                            log_pagamento_confirmado(user, p_info['nome'], data.get('amount', 0)/100, payment_id, item), 
                            bot.loop
                        )
    except Exception as e:
